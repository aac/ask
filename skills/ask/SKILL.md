...
metadata:
  version: "0.2.1"
description: Use when you need a human to take an action an agent can't — set up an OAuth credential, sign into a service, place a file at a path the agent can't reach, give feedback on a UI — and the action outlives the chat the filing agent is in. Also trigger in any repo with a .ask/ directory at its root or an ancestor, when the user mentions "what's in my inbox", "what do you need me to do", "what do you need from me", "anything blocking you", "what are you waiting on me for", "file an ask", "ask request", or ask-XXXX ids, or when an agent uses ask MCP tools (ask_new, ask_list, ask_resolve, ask_reopen, ask_close). Covers the file-resolve-verify-close arc, when to file vs. ask in chat, verifier discipline, and orchestrator checkpoints. Also trigger when an agent generates follow-ups, next steps, or open items in a task, recap, or status report — each item needing a human action outside the chat should be an ask, not a chat bullet. Fire on phrasings like "next steps require the user", "blocked on a decision", "follow-ups need a call", "you'll need to", "the user should", "later you should", or any list of human-action items. Also trigger — even with no `.ask/` directory present anywhere — whenever a human-needed item exists and no store exists yet, since that absence is not a reason to route the item into a handoff doc, a recap's prose, or a chat-only note instead; initialize one with `ask init` and file into it. The "no store here" observation should end with `ask init`, not with the item living somewhere else.
---
...
- **No `.ask/` store yet, but a human-needed item exists.** "Not initialized here" is not a reason to fall back to a handoff doc, a recap paragraph, or a chat-only note — it's a reason to run `ask init` (see Bootstrap above) first, then file normally. Treat the missing store the same way you'd treat a missing directory for any other artifact you're about to write: create it, don't route around it.
